...
#we need the 2018 data and treatments
...
grun2018 = read.csv("data/Rita_longitudinal/longitudinal_disease_survey_2018.csv") %>% left_join(trtments) %>%
  ...
  mutate(PlantID = paste(plot, tiller, sep = "_"), 
...
filt.obs = grun2018 %>% mutate(survey_date = as.Date(survey_date, format = "%m/%d/%Y")) %>%
...
#####Show that fungicide treatment affects arrival timing 

plant.lev <- filt.obs %>%
  select(plot,fungicide, exp_year, PlantID, survey_date, anthracnose, crown_rust, brown_patch, 
         PC1:PC4, LeafID) %>%
  group_by(PlantID, survey_date, fungicide) %>%
...
first_disease <- plant.lev %>%
  group_by(PlantID, fungicide) %>%
  summarise(
    first_anthracnose = if (any(anthracnose == 1)) min(survey_date[anthracnose == 1]) else as.Date(NA),
    first_crown_rust   = if (any(crown_rust == 1))   min(survey_date[crown_rust == 1])   else as.Date(NA),
    first_brown_patch  = if (any(brown_patch == 1))  min(survey_date[brown_patch == 1])  else as.Date(NA),
    .groups = "drop"
  ) %>%
  pivot_longer(
...
first_disease = first_disease %>% group_by(fungicide) %>%
  mutate(doy = as.numeric(date - min(date)))
ggplot(first_disease) + 
  geom_density(aes(x=doy, fill = fungicide)) + facet_wrap(~disease)

disease_table <- table(first_disease$disease, first_disease$fungicide)
chi <- chisq.test(disease_table)
...
rust.plant = filt.obs %>%
  select(plot, fungicide, exp_year, PlantID, survey_date, anthracnose, crown_rust, brown_patch, 
         PC1:PC4, LeafID) %>%
  group_by(PlantID, plot, survey_date, fungicide, PC1, PC2, PC3, PC4) %>%
  summarise(anthracnose = ifelse(sum(anthracnose) > 0, 1, 0),
         brown_patch = ifelse(sum(brown_patch) > 0, 1, 0), 
         crown_rust = ifelse(sum(crown_rust) > 0, 1, 0)) %>% 
  ungroup() %>%
...
rust.plant$fungicide = relevel(factor(rust.plant$fungicide), ref="Never")
...
anth.plant$fungicide = relevel(factor(anth.plant$fungicide), ref = "Never")
...
bp.plant = filt.obs %>%
  select(plot, exp_year, fungicide, PlantID, survey_date, anthracnose, crown_rust, brown_patch, 
         PC1:PC4, LeafID) %>%
  group_by(PlantID, plot, survey_date, fungicide, PC1, PC2, PC3, PC4) %>%
  summarise(anthracnose = ifelse(sum(anthracnose) > 0, 1, 0),
         brown_patch = ifelse(sum(brown_patch) > 0, 1, 0), 
...
bp.plant$fungicide = relevel(factor(bp.plant$fungicide), ref ="Never")
